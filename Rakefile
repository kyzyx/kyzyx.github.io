...
# Usage: rake publish_draft post="A Title" [date="2012-02-09"]
desc "Move draft from #{CONFIG['drafts']} to #{CONFIG['posts']}"
task :publish_draft do
  abort("rake aborted: '#{CONFIG['posts']}' directory not found.") unless FileTest.directory?(CONFIG['posts'])
  abort("rake aborted: '#{CONFIG['drafts']}' directory not found.") unless FileTest.directory?(CONFIG['drafts'])
  abort("rake aborted: draft title required.") unless ENV["title"]
  title = ENV["title"]
  slug = title.downcase.strip.gsub(' ', '-').gsub(/[^\w-]/, '')
  draftfilename = File.join(CONFIG['drafts'], "#{slug}.#{CONFIG['post_ext']}")
  abort("rake aborted: draft with title #{title} not found.") unless File.exist?(draftfilename)
  begin
    date = (ENV['date'] ? Time.parse(ENV['date']) : Time.now).strftime('%Y-%m-%d')
  rescue => e
    puts "Error - date format must be YYYY-MM-DD, please check you typed it correctly!"
  ...
  FileUtils.mv(draftfilename, filename)
end # task :publish_draft
